...
```typescript
try {
  const content = await sandbox.files.read('/missing.txt');
} catch (error) {
  if (error instanceof FileNotFoundError) {
...
```typescript
try {
  await sandbox.files.write('/existing.txt', 'data', { overwrite: false });
} catch (error) {
...
try {
  ...
  await sandbox.files.read('/huge-file.bin');
} catch (error) {
...
```typescript
try {
  await sandbox.files.read('/data/input.json');
} catch (error) {
  if (error instanceof FileNotFoundError) {
